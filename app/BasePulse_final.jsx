...
/**
 * BasePulse.jsx – v5
 * - Real RainbowKit ConnectButton
 * - Trust score with retry (handles Render cold start)
 * - Check-in: once per day per wallet, streak counter, stored in localStorage
 * - Check-in sends ETH via eth_sendTransaction directly (no chainId enforcement)
 * - Farcaster Frame SDK hydration
 */
...
const CHECKIN_WEI     = "0x" + (30000000000000n).toString(16); // 0.00003 ETH in hex wei

// Receipt contract – replace with your deployed contract
...
const RECEIPT_CONTRACT_ABI = [
  { name:"mintReceipt", type:"function", stateMutability:"nonpayable",
    inputs:[{name:"txHash",type:"bytes32"},{name:"label",type:"string"}], outputs:[] },
];

// ── Design tokens ─────────────────────────────────────────────────────────────
const T = {
  bg:"#F9FAFB", surface:"#FFFFFF", surfaceHover:"#F3F4F6",
  border:"rgba(0,0,0,0.06)", text:"#111827", textSub:"#6B7280", textMuted:"#9CA3AF",
  blue:"#0052FF", blueLight:"#EEF3FF", blueMid:"#3B73FF",
...
// ── Seed data ─────────────────────────────────────────────────────────────────
const MOCK_PROFILE = {
  fid:0, username:"—", displayName:"Connect Wallet",
  pfp:null, followers:0, following:0,
  trustScore:0, receipts:0, liquidity:0, verified:false,
};
...
// ── Hook: useCheckIn ──────────────────────────────────────────────────────────
// Manages once-per-day check-in logic and streak counter.
// Stores data in localStorage keyed by wallet address.
function useCheckIn(address) {
  const storageKey = address ? `basepulse_checkin_${address.toLowerCase()}` : null;

  const getState = useCallback(() => {
    if (!storageKey || typeof window === "undefined") return { streak:0, lastDate:null, totalCheckIns:0 };
    try {
      return JSON.parse(localStorage.getItem(storageKey) || "{}");
    } catch { return {}; }
  }, [storageKey]);

  const [checkInState, setCheckInState] = useState(() => ({ streak:0, lastDate:null, totalCheckIns:0 }));

  // Load from storage on address change
  useEffect(() => {
    if (!storageKey) return;
    setCheckInState(s => ({ streak:0, lastDate:null, totalCheckIns:0, ...getState() }));
  }, [storageKey, getState]);
  ...
  const recordCheckIn = useCallback(() => {
    ...
    const newStreak = state.lastDate === yesterday ? (state.streak || 0) + 1 : 1;
    const newState = {
      streak: newStreak,
      lastDate: today,
      totalCheckIns: (state.totalCheckIns || 0) + 1,
    };
  ...
  return { checkInState, canCheckInToday, recordCheckIn };
}

// ── Hook: useFarcasterContext ─────────────────────────────────────────────────
function useFarcasterContext() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { /* not in a frame */ }
      finally { if (!cancelled) setFrameReady(true); }
    })();
...
function useBaseNetwork() {
  const [data, setData] = useState({ blockNumber:null, gasPriceGwei:null, prevGasPrice:null, loading:true, lastUpdated:null });
  const fetch_ = useCallback(async () => {
    try {
      const [rawGas, rawBlock] = await Promise.all([rpcCall("eth_gasPrice"), rpcCall("eth_blockNumber")]);
      setData(prev => ({
        blockNumber: parseInt(rawBlock,16).toLocaleString(),
        gasPriceGwei: (parseInt(rawGas,16)/1e9).toFixed(4),
...
const GlobalStyles = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes pulse-ring{0%{transform:scale(.95);opacity:.8}70%{transform:scale(1.15);opacity:0}100%{transform:scale(1.15);opacity:0}}
    @keyframes spin{to{transform:rotate(360deg)}}
    .spin{animation:spin .8s linear infinite}
    .shimmer-box{background:linear-gradient(90deg,#f0f0f0 25%,#e0e0e0 50%,#f0f0f0 75%);background-size:400px 100%;animation:shimmer 1.4s infinite}
...
function Avatar({ name, pfp, size=40 }) {
  ...
  return <div style={{width:size,height:size,borderRadius:"50%",background:`linear-gradient(135deg,${T.blue},${T.blueMid})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:size*0.35,fontWeight:600,flexShrink:0}}>{initials}</div>;
}
function Badge({ children, color=T.blue, bg=T.blueLight }) {
  return <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:6,fontSize:11,fontWeight:600,color,background:bg}}>{children}</span>;
}
...
function DeltaPill({ current, prev }) {
  ...
  return <span style={{fontSize:10,fontWeight:600,padding:"1px 5px",borderRadius:4,background:up?T.redLight:T.greenLight,color:up?T.red:T.green}}>{up?"↑":"↓"}</span>;
}

// ── PulseGauge ────────────────────────────────────────────────────────────────
function PulseGauge({ value=0 }) {
  const [d,setD]=useState(0);
  useEffect(()=>{
    const end=value,dur=1600,t0=performance.now();let raf;
    const tick=now=>{const p=Math.min((now-t0)/dur,1),e=1-Math.pow(1-p,3);setD(e*end);if(p<1)raf=requestAnimationFrame(tick);};
    raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf);
  },[value]);
  const sz=220,sw=14,r=(sz-sw)/2,cx=sz/2,cy=sz/2,circ=Math.PI*r;
  const arc=(a1,a2,rad)=>{const toR=x=>x*Math.PI/180;return`M ${cx+rad*Math.cos(toR(a1))} ${cy+rad*Math.sin(toR(a1))} A ${rad} ${rad} 0 0 1 ${cx+rad*Math.cos(toR(a2))} ${cy+rad*Math.sin(toR(a2))}`;};
  const path=arc(180,0,r),fill=circ*d,vel=Math.round(d*100),col=d<0.4?T.red:d<0.65?T.amber:T.green,lbl=d<0.4?"Low":d<0.65?"Moderate":"Strong";
  return (
      ...
      <div style={{position:"relative",width:sz,height:sz/2+32}}>
        <svg width={sz} height={sz/2+sw} viewBox={`0 0 ${sz} ${sz/2+sw}`} style={{overflow:"visible"}}>
          ...
          <motion.path d={path} fill="none" stroke={col} strokeWidth={sw} strokeLinecap="round"
            strokeDasharray={`${circ} ${circ}`} initial={{strokeDashoffset:circ}} animate={{strokeDashoffset:circ-fill}}
            transition={{duration:1.6,ease:[.25,.46,.45,.94]}} style={{filter:`drop-shadow(0 0 6px ${col}60)`}}/>
          {[0,.25,.5,.75,1].map(v=>{const a=180-v*180,rad2=a*Math.PI/180;return<circle key={v} cx={cx+r*Math.cos(rad2)} cy={cy+r*Math.sin(rad2)} r={2} fill={d>=v?col:"#D1D5DB"} opacity={0.7}/>;  })}
          <motion.g initial={{rotate:0}} animate={{rotate:(180-d*180)-180}} style={{originX:`${cx}px`,originY:`${cy}px`}} transition={{duration:1.6,ease:[.25,.46,.45,.94]}}>
      ...
      <div style={{display:"flex",gap:16,marginTop:16}}>
        {[["Low",T.red],["Moderate",T.amber],["Strong",T.green]].map(([l,c])=>(
          <div key={l} style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:c}}/><span style={{fontSize:11,color:T.textSub,fontWeight:500}}>{l}</span>
          </div>
        ))}
      </div>
    </div>
...
// ── AnimatedScore ─────────────────────────────────────────────────────────────
function AnimatedScore({ target=0 }) {
  const [count,setCount]=useState(0);
  useEffect(()=>{
    let f;const t0=performance.now();
    const tick=now=>{const t=Math.min((now-t0)/1800,1),e=t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;setCount(Math.round(e*target));if(t<1)f=requestAnimationFrame(tick);};
  ...
  const tc=count>=800?"#7C3AED":count>=600?T.amber:count>=400?"#6B7280":"#92400E";
  const tb=count>=800?"#F5F3FF":count>=600?T.amberLight:count>=400?"#F3F4F6":"#FFFBEB";
  return (
    <div style={{display:"flex",alignItems:"baseline",gap:8}}>
      ...
      <Badge color={tc} bg={tb}>{tier}</Badge>
    </div>
  );
}

// ── TrustCard ─────────────────────────────────────────────────────────────────
function TrustCard({ profile, scoreLoading }) {
  return (
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.1}}
      ...
      <div style={{background:`linear-gradient(135deg,${T.blue}08,${T.blue}04)`,padding:"28px 28px 20px",borderBottom:`1px solid ${T.border}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          {profile.fid>0&&<div style={{textAlign:"right"}}><div style={{fontSize:11,color:T.textMuted,fontWeight:500,marginBottom:4}}>FID</div><div style={{fontSize:13,color:T.text,fontFamily:"'Geist Mono',monospace",fontWeight:600}}>#{profile.fid.toLocaleString()}</div></div>}
        </div>
        {profile.followers>0&&(
      ...
      <div style={{padding:"24px 28px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div style={{fontSize:12,color:T.textSub,fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase"}}>Trust Score</div>
          {scoreLoading&&<div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:T.textSub}}><Spinner size={10} color={T.blue}/>Loading…</div>}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:1,background:T.border,borderRadius:16,overflow:"hidden",marginTop:20}}>
          {[["Receipts",profile.receipts??0,"📋"],["Disputes","0","🛡️"],["Streak","—","🔥"]].map(([l,v,i])=>(
...
// ── CheckInCard ───────────────────────────────────────────────────────────────
function CheckInCard({ address, walletConnected, onSuccess }) {
  const { checkInState, canCheckInToday, recordCheckIn } = useCheckIn(address);
  const [status, setStatus] = useState("idle"); // idle | pending | confirming | success | already | error
  ...
  useEffect(() => {
    ...
    const newState = recordCheckIn();
    onSuccess?.({ txHash, streak: newState.streak, total: newState.totalCheckIns });
    setStatus("success");
  }, [isConfirmed, txHash]); // eslint-disable-line

  useEffect(() => {
    if (isConfirming && status !== "confirming") setStatus("confirming");
  }, [isConfirming]); // eslint-disable-line

  const handleCheckIn = async () => {
    if (!walletConnected || !address) return;
    if (!canCheckInToday()) { setStatus("already"); return; }

    setStatus("pending");
    setErrMsg("");

    try {
      // Use window.ethereum directly — avoids wagmi chain enforcement issues
      ...
      const hash = await provider.request({
        ...
        params: [{
          ...
          value: CHECKIN_WEI,
          // No chainId here — let the wallet use its current chain
        }],
      });
      setTxHash(hash);
    } catch (e) {
      setErrMsg(e?.message?.includes("rejected") || e?.code === 4001
  ...
  const alreadyToday = !canCheckInToday();
  const { streak=0, totalCheckIns=0 } = checkInState;
  ...
  return (
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.3}}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <div>
          <div style={{fontSize:16,fontWeight:700,color:T.text,letterSpacing:"-0.02em"}}>Daily Check-In</div>
          <div style={{fontSize:12,color:T.textSub,marginTop:3}}>Leave a ~$0.01 on-chain footprint on Base</div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        {[["🔥 Streak",`${streak} day${streak!==1?"s":""}`],["✅ Total",`${totalCheckIns} check-in${totalCheckIns!==1?"s":""}`]].map(([l,v])=>(
          <div key={l} style={{background:T.bg,borderRadius:12,padding:"12px 14px"}}>
            <div style={{fontSize:11,color:T.textSub,fontWeight:500,marginBottom:4}}>{l}</div>
      ...
      {/* Status messages */}
      {alreadyToday&&status!=="success"&&(
        <div style={{background:T.greenLight,border:`1px solid ${T.green}30`,borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:T.green,fontWeight:500}}>
          ✓ Already checked in today — come back tomorrow to keep your streak!
        </div>
      )}
      {errMsg&&(
        <div style={{background:T.redLight,border:`1px solid ${T.red}30`,borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:T.red,fontWeight:500}}>
          {errMsg}
        </div>
      )}
      {!walletConnected&&(
        <div style={{background:T.amberLight,border:`1px solid ${T.amber}30`,borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:T.amber,fontWeight:500}}>
          ⚠️ Connect your wallet to check in
        </div>
      )}

      <motion.button
        onClick={handleCheckIn}
        disabled={!walletConnected||status==="pending"||status==="confirming"||status==="success"||alreadyToday}
        whileHover={walletConnected&&!alreadyToday?{scale:1.02}:{}}
        whileTap={walletConnected&&!alreadyToday?{scale:.98}:{}}
        style={{
          width:"100%",padding:"13px",borderRadius:14,border:"none",
          background: status==="success"?T.green
            : alreadyToday?T.surfaceHover
            : walletConnected?`linear-gradient(135deg,${T.purple},#6D28D9)`
            :"#E5E7EB",
          color: alreadyToday?T.textMuted : walletConnected?"#fff":T.textMuted,
          fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:8,
          boxShadow:walletConnected&&!alreadyToday&&status!=="success"?`0 4px 16px ${T.purple}40`:"none",
          transition:"all 0.2s",
        }}>
        {status==="pending"    ? <><Spinner/>Confirm in wallet…</>
        :status==="confirming" ? <><Spinner/>Anchoring to Base…</>
        :status==="success"    ? <>✓ Checked In! Streak: {streak} 🔥</>
        :alreadyToday          ? <>✓ Checked in today</>
        :                        <>📍 Check In (~$0.01 ETH)</>}
      </motion.button>

      <div style={{marginTop:10,fontSize:11,color:T.textMuted,textAlign:"center"}}>
        Sends 0.00003 ETH to your own wallet · once per day
      </div>
    </motion.div>
  );
}
...
function ReceiptForm({ onMint, walletConnected }) {
  ...
  const { isLoading:isConfirming, isSuccess:isConfirmed } = useWaitForTransactionReceipt({
    hash:submittedHash, chainId:BASE_CHAIN_ID, query:{enabled:!!submittedHash},
  });
  ...
  useEffect(()=>{
    ...
    setStatus("success");
    const t=setTimeout(()=>{setStatus("idle");setHash("");setLabel("");setSubmittedHash(null);},2600);
  ...
  const handleSubmit=async()=>{
    ...
    try {
      ...
      // Encode the call manually since we can't use useWriteContract without a real contract
      // For now, send a minimal ETH transfer as a placeholder until contract is deployed
      const txHash=await provider.request({
        method:"eth_sendTransaction",
        params:[{from:walletConnected,to:RECEIPT_CONTRACT_ADDRESS,value:"0x0",data:"0x"}],
      });
      setSubmittedHash(txHash);
      setStatus("confirming");
    } catch(e){
      setError(e?.code===4001?"Transaction cancelled.":e?.message||"Failed.");
      setStatus("idle");
    }
  };
  ...
  return (
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.25}}
      ...
      {!walletConnected&&<div style={{background:T.amberLight,border:`1px solid ${T.amber}30`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:12,color:T.amber,fontWeight:500}}>⚠️ Connect your wallet to mint a receipt</div>}
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,background:T.bg,borderRadius:12,padding:"12px 14px",border:`1.5px solid ${error?T.red:hash&&isValid(hash)?T.green:T.border}`}}>
            ...
            <input value={hash.startsWith("0x")?hash.slice(2):hash}
              ...
              style={{flex:1,background:"transparent",border:"none",fontSize:13,fontFamily:"'Geist Mono',monospace",color:T.text}}/>
        ...
        <div>
          ...
          <input value={label} onChange={e=>setLabel(e.target.value)} placeholder="e.g. Swap on Uniswap…"
            style={{width:"100%",background:T.bg,border:`1.5px solid ${T.border}`,borderRadius:12,padding:"12px 14px",fontSize:13,color:T.text}}
            onFocus={e=>e.target.style.borderColor=T.blue} onBlur={e=>e.target.style.borderColor=T.border}/>
        </div>
        <motion.button onClick={handleSubmit} disabled={status==="pending"||status==="confirming"||status==="success"}
          whileHover={{scale:1.01}} whileTap={{scale:.98}}
          style={{width:"100%",padding:"14px",borderRadius:14,border:"none",background:status==="success"?T.green:`linear-gradient(135deg,${T.blue},#3B73FF)`,color:"#fff",fontSize:14,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:T.shadowBlue,transition:"background 0.3s"}}>
          {status==="pending"?<><Spinner/>Confirm in wallet…</>:status==="confirming"?<><Spinner/>Anchoring to Base…</>:status==="success"?<>✓ Receipt Minted</>:<>⚡ Mint Receipt</>}
        </motion.button>
      </div>
    </motion.div>
  );
  ...
  return (
    <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{delay:index*0.07}}
      ...
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:36,height:36,borderRadius:10,flexShrink:0,background:holding?T.amberLight:T.greenLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{holding?"⏳":"✅"}</div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
            <span style={{fontSize:13,fontWeight:600,color:T.text}}>{receipt.label}</span>
        ...
        <div style={{position:"relative"}}>
          <motion.button whileHover={{scale:1.1}} whileTap={{scale:.9}} onClick={()=>!flagged&&setShowDispute(v=>!v)}
            style={{width:28,height:28,borderRadius:8,border:"none",background:flagged?T.amberLight:T.surfaceHover,color:flagged?T.amber:T.textMuted,fontSize:13,display:"flex",alignItems:"center",justifyContent:"center"}}>🚩</motion.button>
          <AnimatePresence>
            {showDispute&&(
              <motion.div initial={{opacity:0,scale:.9,y:-4}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.9,y:-4}} transition={{duration:.15}}
                ...
                <div style={{fontSize:11,color:T.textSub,marginBottom:12,lineHeight:1.5}}>Places a 24-hour hold on the receipt pending review.</div>
...
function NetworkStats() {
  ...
  return (
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:.35}}
      style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:600,color:T.text}}>Base Network <span style={{fontSize:11,color:T.textSub,fontWeight:400}}>· live</span></div>
        {loading&&<Spinner size={12} color={T.blue}/>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {stats.map((s,i)=>(
          <motion.div key={s.label} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.4+i*.06}} style={{background:T.bg,borderRadius:14,padding:"14px"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              <span style={{fontSize:14}}>{s.icon}</span><span style={{fontSize:11,color:T.textSub,fontWeight:500}}>{s.label}</span>
            </div>
            <div style={{display:"flex",alignItems:"baseline",gap:6}}>
              {loading&&!s.value?<span className="shimmer-box" style={{display:"inline-block",width:60,height:18,borderRadius:4}}/>
...
function FrameContextBanner({ frameCtx }) {
  ...
  return (
    <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} transition={{delay:.6}}
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:12,fontWeight:600,color:T.blue}}>{isReal?"Farcaster Frame — Verified Context":"Farcaster Frame Active"}</div>
        <div style={{fontSize:11,color:T.textSub,marginTop:1}}>{isReal?`Viewing as ${frameCtx.displayName} · FID #${frameCtx.fid}`:"Open via Warpcast to load real identity"}</div>
      </div>
...
// ── Views ─────────────────────────────────────────────────────────────────────
function DashboardView({ profile, receipts, onMint, onFlag, walletConnected, address, onCheckIn, scoreLoading }) {
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:20}}>
      <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <TrustCard profile={profile} scoreLoading={scoreLoading}/>
        <ReceiptForm onMint={onMint} walletConnected={walletConnected}/>
        <CheckInCard address={address} walletConnected={walletConnected} onSuccess={onCheckIn}/>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5}}
          ...
          <div style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
            <div><div style={{fontSize:16,fontWeight:700,color:T.text}}>Liquidity Velocity</div><div style={{fontSize:12,color:T.textSub,marginTop:3}}>Real-time pulse meter</div></div>
            <div style={{position:"relative"}}><div style={{width:10,height:10,borderRadius:"50%",background:T.green,position:"absolute",animation:"pulse-ring 2s infinite"}}/><div style={{width:10,height:10,borderRadius:"50%",background:T.green}}/></div>
          </div>
          <PulseGauge value={profile.liquidity||0}/>
        </motion.div>
        <NetworkStats/>
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.3}}
          style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"28px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            <div><div style={{fontSize:16,fontWeight:700,color:T.text}}>Verified Receipts</div><div style={{fontSize:12,color:T.textSub,marginTop:2}}>{receipts.length} total · {receipts.filter(r=>r.status!=="flagged").length} verified</div></div>
            <Badge color={T.blue} bg={T.blueLight}>{receipts.length}</Badge>
          </div>
          <AnimatePresence>{receipts.map((r,i)=><ReceiptItem key={r.hash+i} receipt={r} index={i} onFlag={onFlag} total={receipts.length}/>)}</AnimatePresence>
        </motion.div>
      </div>
    </div>
  );
...
function ReceiptsView({ receipts, onFlag }) {
  ...
  return (
    <motion.div key="receipts" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
      <div style={{marginBottom:24}}><h2 style={{fontSize:22,fontWeight:700,color:T.text}}>Receipts</h2><p style={{fontSize:13,color:T.textSub,marginTop:4}}>{receipts.length} total · {verified} verified · {receipts.length-verified} in review</p></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:20}}>
        {[{label:"Total Value",val:"$1,708",icon:"💰",color:T.blue,bg:T.blueLight},{label:"Verified",val:verified,icon:"✅",color:T.green,bg:T.greenLight},{label:"Under Review",val:receipts.length-verified,icon:"⏳",color:T.amber,bg:T.amberLight}].map(c=>(
          <motion.div key={c.label} initial={{opacity:0,scale:.97}} animate={{opacity:1,scale:1}} style={{background:T.surface,borderRadius:18,boxShadow:T.shadow,padding:"20px 22px",display:"flex",alignItems:"center",gap:14}}>
            ...
            <div><div style={{fontSize:22,fontWeight:700,color:T.text}}>{c.val}</div><div style={{fontSize:12,color:T.textSub}}>{c.label}</div></div>
          </motion.div>
        ))}
      </div>
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.15}} style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"28px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontSize:15,fontWeight:700,color:T.text}}>All Receipts</div><Badge color={T.blue} bg={T.blueLight}>{receipts.length}</Badge>
        </div>
        <AnimatePresence>{receipts.map((r,i)=><ReceiptItem key={r.hash+i} receipt={r} index={i} onFlag={onFlag} total={receipts.length}/>)}</AnimatePresence>
      </motion.div>
    </motion.div>
...
function NetworkView() {
  return (
    <motion.div key="network" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
      <div style={{marginBottom:24}}><h2 style={{fontSize:22,fontWeight:700,color:T.text}}>Network</h2><p style={{fontSize:13,color:T.textSub,marginTop:4}}>Live Base mainnet data · refreshes every 10s</p></div>
      <NetworkStats/>
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.2}} style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"28px",marginTop:20}}>
...
function SettingsView({ profile, connected, address }) {
  ...
  const Toggle=({val,set,label,sub})=>(
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 0",borderBottom:`1px solid ${T.border}`}}>
      <div><div style={{fontSize:14,fontWeight:500,color:T.text}}>{label}</div>{sub&&<div style={{fontSize:12,color:T.textSub,marginTop:2}}>{sub}</div>}</div>
  ...
  return (
    <motion.div key="settings" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
      <div style={{marginBottom:24}}><h2 style={{fontSize:22,fontWeight:700,color:T.text}}>Settings</h2><p style={{fontSize:13,color:T.textSub,marginTop:4}}>Manage your BasePulse preferences</p></div>
      <div style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px",marginBottom:20}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,padding:"0 0 16px",borderBottom:`1px solid ${T.border}`}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:15,fontWeight:600,color:T.text}}>{profile.displayName}</div>{profile.fid>0&&<div style={{fontSize:12,color:T.textSub,fontFamily:"'Geist Mono',monospace"}}>FID #{profile.fid}</div>}</div>
        ...
        <div style={{paddingTop:16,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div><div style={{fontSize:14,fontWeight:500,color:T.text}}>Wallet</div><div style={{fontSize:12,color:T.textSub,marginTop:2,fontFamily:"'Geist Mono',monospace"}}>{connected&&address?`${address.slice(0,6)}…${address.slice(-4)}`:"Not connected"}</div></div>
          {connected?<button onClick={()=>disconnect()} style={{fontSize:12,color:T.red,background:T.redLight,border:"none",padding:"6px 12px",borderRadius:8,fontFamily:"inherit",fontWeight:600}}>Disconnect</button>:<Badge color={T.amber} bg={T.amberLight}>⚠️ Not connected</Badge>}
        </div>
      </div>
      <div style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px",marginBottom:20}}>
      ...
      <div style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px"}}>
        <div style={{fontSize:12,fontWeight:600,color:T.textSub,letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:16}}>App Info</div>
        {[["Version","5.0.0"],["Chain","Base Mainnet (8453)"],["Trust API","basepulse-trust.onrender.com"],["Frame SDK","@farcaster/frame-sdk"]].map(([k,v])=>(
...
export default function BasePulse() {
  ...
  const profile = {
    ...MOCK_PROFILE,
    ...(liveProfile||{}),
    ...(frameCtx?{fid:frameCtx.fid,displayName:frameCtx.displayName,username:frameCtx.username,pfp:frameCtx.pfp}:{}),
    ...(isConnected&&address&&!frameCtx?{displayName:`${address.slice(0,6)}…${address.slice(-4)}`,username:address.slice(0,10)}:{}),
  };
  ...
  const handleMint = useCallback(({hash,label})=>{
    const sh=hash.length>20?`${hash.slice(0,10)}…${hash.slice(-6)}`:hash;
    setReceipts(prev=>[{hash:sh,label,ts:"just now",status:"verified",amount:"+$?",isNew:true},...prev]);
    toast("Receipt minted on Base ✓");
  },[toast]);

  const handleFlag    = useCallback(()=>toast("24-hour hold initiated ⏳","warning"),[toast]);
  const handleCheckIn = useCallback(({txHash,streak,total})=>{
    setReceipts(prev=>[{hash:txHash,label:`Daily Check-In · Day ${total}`,ts:"just now",status:"verified",amount:"-0.00003 ETH",isNew:true},...prev]);
    toast(`Check-in confirmed! 🔥 ${streak}-day streak`);
  },[toast]);
  ...
  const renderView=()=>{
    switch(activeNav){
      case "pulse":    return <DashboardView profile={profile} receipts={receipts} onMint={handleMint} onFlag={handleFlag} walletConnected={isConnected} address={address} onCheckIn={handleCheckIn} scoreLoading={scoreLoading}/>;
      case "receipts": return <ReceiptsView receipts={receipts} onFlag={handleFlag}/>;
      case "network":  return <NetworkView/>;
      case "settings": return <SettingsView profile={profile} connected={isConnected} address={address}/>;
      default:         return null;
    }
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Geist','Inter',sans-serif"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px 100px"}}>
        <div style={{marginBottom:20}}><FrameContextBanner frameCtx={frameCtx}/></div>
        {activeNav==="pulse"&&<div style={{marginBottom:28}}><h1 style={{fontSize:28,fontWeight:700,color:T.text,letterSpacing:"-0.04em",lineHeight:1.1}}>Reputation Dashboard</h1><p style={{fontSize:14,color:T.textSub,marginTop:6}}>Your on-chain identity layer on Base</p></div>}
        ...
        {navItems.map(item=>{
          ...
          return (
            <button key={item.id} onClick={()=>setActiveNav(item.id)}
              ...
              {active&&<motion.div layoutId="nav-dot" style={{position:"absolute",top:-10,width:4,height:4,borderRadius:"50%",background:T.blue}}/>}
              <motion.div animate={{scale:active?1.18:1,y:active?-1:0}} transition={{type:"spring",stiffness:400,damping:20}} style={{fontSize:20}}>{item.icon}</motion.div>
