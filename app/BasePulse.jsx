"use client";
/**
 * BasePulse.jsx – v4 "Final Production"
 *
 * What's real:
 *  [1] RainbowKit ConnectButton – real wallet connect modal
 *  [2] useAccount() – real address, isConnected from wagmi
 *  [3] useTrustScore – fetches live score from Render API on wallet connect
 *  [4] useBaseNetwork – polls Base RPC every 10s for gas + block
 *  [5] useFarcasterContext – hydrates profile from frame context in Warpcast
 *  [6] Check-In – sends 0.00003 ETH (~$0.01) to self as on-chain footprint
 *  [7] ReceiptForm – useWriteContract wired (needs real contract address)
 *
 * Only one thing left for you to supply:
 *   RECEIPT_CONTRACT_ADDRESS – your deployed receipt contract on Base mainnet
 */
...
// Check-in amount: 0.00003 ETH ≈ $0.01 at current prices
const CHECKIN_AMOUNT = "0.00003";
...
function useFarcasterContext() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { sdk } = await import("@farcaster/frame-sdk").catch(() => ({ sdk: null }));
        ...
        // Not in a Farcaster frame – silent fallback
      } finally {
      ...
      <div style={{background:`linear-gradient(135deg,${T.blue}08,${T.blue}04)`,padding:"28px 28px 20px",borderBottom:`1px solid ${T.border}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{position:"relative"}}>
              ...
              {profile.verified&&(
                <div style={{position:"absolute",bottom:-2,right:-2,width:18,height:18,borderRadius:"50%",background:T.blue,border:`2px solid ${T.surface}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff"}}>✓</div>
              )}
            </div>
            <div>
        ...
        {profile.followers>0&&(
          <div style={{display:"flex",gap:20,marginTop:16}}>
            {[["Followers",profile.followers.toLocaleString()],["Following",profile.following]].map(([label,val])=>(
              <div key={label}>
                <span style={{fontSize:14,fontWeight:600,color:T.text}}>{val}</span>
                <span style={{fontSize:12,color:T.textSub,marginLeft:4}}>{label}</span>
              </div>
            ))}
          </div>
        )}
...
// ── CheckInButton ─────────────────────────────────────────────────────────────
// [6] Sends CHECKIN_AMOUNT ETH to self as an on-chain footprint transaction
function CheckInButton({ address, walletConnected, onSuccess }) {
  const { sendTransaction, isPending, data: txHash, reset } = useSendTransaction();
  ...
  useEffect(() => {
    ...
    onSuccess?.(`${txHash.slice(0,10)}…${txHash.slice(-6)}`);
  ...
  const handleCheckIn = () => {
    ...
    sendTransaction({
      ...
      value: parseEther(CHECKIN_AMOUNT),
      chainId: BASE_CHAIN_ID,
    });
  };
  ...
  return (
    <motion.button
      ...
      disabled={!walletConnected || isLoading || isSuccess}
      whileHover={walletConnected&&!isLoading?{scale:1.02}:{}}
      whileTap={walletConnected&&!isLoading?{scale:.98}:{}}
      style={{
        width:"100%",padding:"13px",borderRadius:14,border:"none",
        background:isSuccess?T.green:walletConnected?`linear-gradient(135deg,#7C3AED,#6D28D9)`:"#E5E7EB",
        color:walletConnected?"#fff":T.textMuted,
        fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:8,
        boxShadow:walletConnected&&!isSuccess?"0 4px 16px rgba(124,58,237,0.3)":"none",
        transition:"all 0.2s",
      }}>
      {isLoading
        ? <><Spinner/>Confirming check-in…</>
        : isSuccess
          ? <>✓ Checked In On-Chain</>
          : <>📍 Check In (~$0.01 ETH)</>}
    </motion.button>
...
function ReceiptForm({ onMint, walletConnected }) {
  ...
  const { writeContract, data: submittedHash, isPending: isSubmitting, reset: resetWrite } = useWriteContract();
  ...
  const handleSubmit = () => {
    ...
    writeContract(
      ...
      { onError: (e) => { setError(e?.shortMessage || e?.message || "Transaction rejected."); resetWrite(); } }
    );
  };
  ...
  return (
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.25}}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div>
          <label style={{fontSize:11,fontWeight:600,color:T.textSub,letterSpacing:"0.04em",textTransform:"uppercase",display:"block",marginBottom:6}}>
            Label <span style={{color:T.textMuted,textTransform:"none",fontWeight:400}}>(optional)</span>
          </label>
        ...
        <motion.button onClick={handleSubmit} disabled={isLoading||isSuccess}
          ...
          {isSubmitting?<><Spinner/>Confirm in wallet…</>:isConfirming?<><Spinner/>Anchoring to Base…</>:isSuccess?<>✓ Receipt Minted</>:<>⚡ Mint Receipt</>}
        </motion.button>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:11,color:T.textSub,marginTop:1}}>
          {isReal
            ...
            :"Open via Warpcast to load real identity"}
        </div>
      </div>
...
// ── Views ─────────────────────────────────────────────────────────────────────
function DashboardView({ profile, receipts, onMint, onFlag, walletConnected, address, onCheckIn }) {
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:20}}>
      <div style={{display:"flex",flexDirection:"column",gap:20}}>
        ...
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.3}}
          style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px"}}>
          <div style={{fontSize:16,fontWeight:700,color:T.text,letterSpacing:"-0.02em",marginBottom:6}}>On-Chain Check-In</div>
          <div style={{fontSize:12,color:T.textSub,marginBottom:16}}>
            Leave a ~$0.01 footprint on Base mainnet. Sends {CHECKIN_AMOUNT} ETH to your own wallet — proves liveness and boosts your activity score.
          </div>
          <CheckInButton address={address} walletConnected={walletConnected} onSuccess={onCheckIn}/>
        </motion.div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:20}}>
  ...
  return (
    <motion.div key="network" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
      <div style={{marginBottom:24}}>
        ...
        <p style={{fontSize:13,color:T.textSub,marginTop:4}}>Live Base mainnet data · refreshes every 10s</p>
      </div>
      <NetworkStats/>
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.2}}
        ...
        <div style={{marginTop:16,fontSize:12,color:T.textSub,lineHeight:1.7}}>
          Data fetched via <code style={{fontFamily:"'Geist Mono',monospace",background:T.bg,padding:"1px 5px",borderRadius:4}}>eth_gasPrice</code> and <code style={{fontFamily:"'Geist Mono',monospace",background:T.bg,padding:"1px 5px",borderRadius:4}}>eth_blockNumber</code> — no API key required.
        </div>
      </motion.div>
    </motion.div>
  );
  ...
  return (
    <motion.div key="settings" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.35}}>
      ...
      <div style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px",marginBottom:20}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,padding:"0 0 16px",borderBottom:`1px solid ${T.border}`}}>
          ...
          <div style={{flex:1}}>
            ...
            {profile.fid>0&&<div style={{fontSize:12,color:T.textSub,fontFamily:"'Geist Mono',monospace"}}>FID #{profile.fid}</div>}
          </div>
        ...
        <div style={{paddingTop:16,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          {connected
            ?<button onClick={()=>disconnect()} style={{fontSize:12,color:T.red,background:T.redLight,border:"none",padding:"6px 12px",borderRadius:8,fontFamily:"inherit",fontWeight:600}}>Disconnect</button>
      ...
      <div style={{background:T.surface,borderRadius:24,boxShadow:T.shadow,padding:"24px 28px"}}>
        ...
        {[["Version","4.0.0 (Production)"],["Chain","Base Mainnet (8453)"],["Trust API","basepulse-trust.onrender.com"],["Frame SDK","@farcaster/frame-sdk"]].map(([k,v])=>(
...
export default function BasePulse() {
  ...
  const { profile: liveProfile } = useTrustScore(address);
  ...
  // Merge: mock defaults → live API data → Farcaster frame identity
  const profile = {
    ...MOCK_PROFILE,
    ...(liveProfile || {}),
    ...(frameCtx ? {
      fid:         frameCtx.fid,
      displayName: frameCtx.displayName,
      username:    frameCtx.username,
      pfp:         frameCtx.pfp,
    } : {}),
    // If wallet connected but no frame context, show address as username
    ...(isConnected && address && !frameCtx ? {
  ...
  const handleCheckIn = useCallback((txHash) => {
    setReceipts(prev=>[{ hash:txHash, label:"On-Chain Check-In", ts:"just now", status:"verified", amount:`-${CHECKIN_AMOUNT} ETH`, isNew:true }, ...prev]);
    toast("Check-in confirmed on Base ✓");
  }, [toast]);
  ...
  const renderView = () => {
    switch (activeNav) {
      case "pulse":
        return <DashboardView profile={profile} receipts={receipts} onMint={handleMint} onFlag={handleFlag} walletConnected={isConnected} address={address} onCheckIn={handleCheckIn}/>;
      case "receipts":
        return <ReceiptsView receipts={receipts} onFlag={handleFlag}/>;
      case "network":
        return <NetworkView/>;
      case "settings":
        return <SettingsView profile={profile} connected={isConnected} address={address}/>;
      default:
        return null;
    }
  };
